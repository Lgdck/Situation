...
//import edu.harvard.syrah.sbon.async.comm.http.HTTPComm.AcceptConnHandler;
...
public class ObjComm extends TCPComm implements ObjCommIF {
	...
	private static final int MAX_OUTSTANDING_CBS = 10; // was: 10
...
//		EL.get().handleNetwork();
	...
	class ReadConnHandler extends TCPComm.ReadConnHandler {
		...
		ReadConnHandler(SelectableChannel channel, NetAddress destAddr) {
			super(channel, destAddr);  //最终调用的是Comm里的 ConnHandler类的构造函数
		}
		...
		public Boolean cb(CBResult result, SelectionKey key) {
			...
			if (key.isValid() && key.isReadable()) {
				...
				if (buffer.position() < MAX_BUFFER_SIZE) {
					...
					if (buffer.remaining() == 0) {  //limit-position
...
//						System.out.println("position: "+ buffer.position() +"  count: "+count);
//						System.out.println(channel);
					...
					if (count == -1 ) {
						...
						closeConnection(remoteAddr, socketChannel);  //在这一步关闭的连接  怎么走到的这一步
						return true;
					}
				} else {
	...
	@Override
	protected edu.harvard.syrah.sbon.async.comm.Comm.AcceptConnHandler getAcceptConnHandler(AddressIF localAddress,
			CB0 cbHandler) {
		// TODO Auto-generated method stub
		return new AcceptConnHandler(localAddress, cbHandler);
	}
	...
	public static void main(String[] args) {
		log.debug("Testing object communication");
