...
import util.bloom.CHTE.parse.Util.DataUtils;
import util.bloom.CHTE.parse.Util.HashAlgorithm;
import util.bloom.CHTE.parse.Util.IPUtils;
import util.bloom.CHTE.parse.entity.IPHeader;
import util.bloom.CHTE.parse.entity.NBStructure;
import util.bloom.CHTE.parse.entity.PacketHeader;
import util.bloom.CHTE.parse.entity.TCPHeader;
...
//                if(packetHeader.getCapLen()>1500)   continue;
                ...
                String mess = nb.getId() + "                       " +nb.getWireSharkTs()+"                  "+ nb.gettS() + "                                          " + nb.getTcpPayLoadLen();
                out.write((mess+"\n").getBytes());
            }
    ...
    private void parsePacketData(byte[] packetDataBuffer,NBStructure nb) {
        ...
        //如果是隧道协议包含的tcp  这里的DataBuffer由14  改成14+0
        ...
        nb.setTcpPayLoadLen(packetDataBuffer.length-54);
    ...
    public void chuliFileTimeCha(File file,OutputStream out,int id) throws IOException {
        ...
        while (sc.hasNext()){
            ...
            BigDecimal curTs=new BigDecimal(tokenizer.nextToken());
            BigDecimal curWirSharkTs=new BigDecimal(tokenizer.nextToken());
            ...
            if(flag){
                out.write((curId +"           "+(curTs.subtract(start)).toString()+"       "+(curWirSharkTs.toString())+"       "+len   +"\n").getBytes());
            }
        }
...
//        System.out.println(seqmess +"   "+ ackmess);
    ...
    private PacketHeader parsePacketHeader(byte[] dataHeaderBuffer, NBStructure nb, int idx){
        ...
        instance.setMaximumFractionDigits(6); //保留20位小数
        ...
        String wireSharkTS = instance.format((packetHeader.getTimeS() + packetHeader.getTimeMs() / 1000000.0) / 1.0);
//        System.out.println(wireSharkTS);
        //还是用抓包的时钟  不要纳秒
        StringBuilder sb=new StringBuilder();
        String TS = sb.append(timeS).append(".").append(String.format("%06d",timeMs)).toString();
