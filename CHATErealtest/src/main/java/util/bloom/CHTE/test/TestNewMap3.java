...
public class TestNewMap3 {
    ...
    public int accur = 2047;//精度为后4位15，后5位31，后6位,63;后7位，127；后8位，255；后9位511后10位1023，后11位2047。默认后8位
...
//                System.out.println("————————" + id + "没到了");
    ...
    public List<String> chargeReceiver_A_new1(int i,List<String> receiveList_ii) {
//        int[] range=getStart(i);//截取的起始位置
        List<String> chargeSuccess=new ArrayList<>();

        List<String> receiveList_i = receiveList_ii;
        int start_receiveList_i=0;
//        double count_receiver_useful = 0;
//        double tsSum_receiver_useful = 0;
        double temp_sum = 0;
...
//            System.out.println("该记录被插入到了第" + row);
    ...
    void setMapSender(List<String> i) {
        ...
        if (Math.pow(2, a) < temp)
            Mod_send = (int) Math.pow(2, a + 2);
        else
            Mod_send = (int) Math.pow(2, a+1);
    }
    void setMapSender1(int i) {
        Mod_send=Mod_receive;

    }
    void setMapReceiver1(List<String> i) {

        int temp = i.size() / (MaxRowHave-1);
        int a = Integer.toBinaryString(temp).length();
        if (Math.pow(2, a) < temp)
            Mod_receive = (int) Math.pow(2, a + 2);
        else
...
//            if(ts_send>i*1+1)
    ...
    public List<String> setRange1(int i, int range_start) {
        List<String> receiveList_i = receiveList.subList(range_start, receiveList.size());
        int number=0;
        int number1=0;
        while(receiveList_i.size()!=0){
            String readRow=receiveList_i.get(number);
            StringTokenizer st=new StringTokenizer(readRow);
            long id=0;
            double ts_send=0;
            int state = 0;
            ...
            if (ts_send>=i*1&&ts_send<=i*1+2){
                number++;
            }  else if(ts_send>i*1+2)
                break;
        }
        receiveList_i=receiveList_i.subList(number1,number);
        return receiveList_i;
...
//            System.out.println(j);
            TestNewMap3 test = new TestNewMap3();
...
//            String fileB = basic + nu + "/sortReceive.txt";
            ...
            //第一个测量节点的数据
            String file1 = "/Users/codersavior/Documents/study project/HuaWei Paper/realtest1116/send2.txt";
            //第二个测量节点的数据
            String file2 = "/Users/codersavior/Documents/study project/HuaWei Paper/realtest1116/server2.txt";
            //第三个测量节点的数据
//            String file3 = "/Users/codersavior/Documents/study project/HuaWei Paper/realtest721/test2/test2-44.txt";
            //第四个测量节点的数据
            String file4 = "/Users/codersavior/Documents/study project/HuaWei Paper/realtest729/729_out/server_test3.txt";
            String fileA="";
            String fileB="";
            switch (j){
                case 0:
            ...
            int range_start=0;
            int range_start_receive=0;
            int max_effect = 1;
            int receive_start = 0;//接收端插入B起始编号。
            double result[][] = new double[600][5];
            double temp_sum=0;
            double temp_sum_pow=0;
            double count_sum=0;
            double var_avg=0;
            int lala=0;
            for (int i = 0; i <600; i++) {//表示处理多少秒
                System.out.println("第"+i+"s");
                List<String> receiveList_i=test.setRange1(i,range_start_receive);
                List<String> sendList_i=test.setRange(i,range_start);
//                range_start_receive+=receiveList_i.size();
                range_start_receive=0;
                range_start+=sendList_i.size();
                double ts_useful_Receiver_A=0;
                double result_Sender_B[];
                List<String> result_Receiver_A;
                if (receiveList_i.size()>sendList_i.size()){
                    test.setMapReceiver1(receiveList_i);
                    test.parseFileReceive(i, receiveList_i);
                    result_Sender_B = test.chargeSender_B(i,sendList_i);
                    //设置CHATE桶数量
                    test.setMapSender1(i);
//                test.setMapSender(sendList_i);
                    test.parseFileSend(sendList_i,i);//把send中list添加到结构mapSend中
                    result_Receiver_A = test.chargeReceiver_A_new1(i,receiveList_i);//第一个值是有用包的个数，第二个值是有用数据包时间戳的和,第三个值是切分位置。
                    ts_useful_Receiver_A=test.getTSsum(result_Receiver_A);
...
//                double[] result_Sender_B = test.chargeSender_B(i,sendList_i);//第一个值是前一百个的有用值，第二个值是前一百有用值时间戳的和
                    ...
                    test.setMapSender(sendList_i);
                    test.parseFileSend(sendList_i,i);//把send中list添加到结构mapSend中
                    result_Receiver_A = test.chargeReceiver_A_new(i);//第一个值是有用包的个数，第二个值是有用数据包时间戳的和,第三个值是切分位置。
                    ts_useful_Receiver_A=test.getTSsum(result_Receiver_A);
//            System.out.println(result_Receiver_A);
                    test.setMapReceiver(i, result_Receiver_A);
                    test.parseFileReceive(i, result_Receiver_A);//把receive中list添加到结构mapReceive中,第二个参数是开始位置，第三个参数是结束位置
                    result_Sender_B = test.chargeSender_B(i,sendList_i);//第一个值是前一百个的有用值，第二个值是前一百有用值时间戳的和
                }
