...
public class PairRDAHost {
    ...
    // Imperial blocks ports outside of 55000-56999  阻止 55000-56999 之外的端口?
    ...
    public static final double scaleTS = Double.parseDouble(Config
            .getConfigProps().getProperty("scaleTS", "1000"));
    ;//Math.pow(10, 1);//Math.pow(10, -11);
    ...
    //duplicate sets
    HashSet<Long> _HostACache;  //这俩没用到
    HashSet<Long> _HostBCache;

    //comm end point
    private ObjCommIF comm;

    public static int Repeat_DELAY = 1000;  //测量间隔1s？
    // tick control
    ...
    public PairRDAHost(ObjCommIF _comm) {
        ...
        increaser = 1;
        comm = _comm;
        _hostFourFields = new RDAMeasurementPoint(hashFuncNum * Math.max(694371 - 313999, RDAEntries), TotalPackets, hashFuncNum); //RDAEntries 5000
        ...
        try {
            AddressFactory.createResolved(Arrays.asList(myRegion),
                    COMM_PORT, new CB1<Map<String, AddressIF>>() {
                        //所有的CallBack接口的回调方法  一般都是通过.call方法call(CBXXXX.OK(),这个静态方法返回一个CB类)
                        protected void cb(CBResult result,
    ...
    public boolean parseTimeStampFromWireshark(String str, long[] id, double[] TS) {
        ...
        if (!goodPacket) {
            ...
            try {
                //string
                str = sb.toString();
                id[0] = MapFromTCP2HashID(str); //报文信息的hash值  第一次hash 报文信息的预处理 生成一个hash ID
    ...
    public static void parseTimeStampFromTCPExtract(String str, long[] id, double[] TS) {
        ...
        try {
            //string
            str = sb.toString();  //str是报文的简要信息   包括源ip 目的ip Ssq Ack Win Len
            //用的SimpleLongHash的hash算法
    ...
    public void registerRepeatRDATestTimer() {
        long delay = Repeat_DELAY; //间隔 设的1s  真实有效
    ...
    public class QueryRDAReqHandler extends ResponseObjCommCB<RDARequestMsg> {
        ...
        //ResponseObjCommCB 继承 ObjCommCB 继承CB4传输块 重写抽象方法
        @Override
        protected void cb(CBResult result, RDARequestMsg arg1, AddressIF arg2, Long arg3, CB1<Boolean> cbHandled) {
            // TODO Auto-generated method stub
            final AddressIF fromNode = arg1.from;  //from 是 一端通信的AddressIf
            //store my hashtable
            RDAResponseMsg msg;
            if (arg1.isA) {
                log.main("B's RDA!");
            ...
            protected void cb(CBResult result, AddressIF addr) {
                ...
                switch (result.state) {
                    case OK: {
                        ...
                        AddressIF neighbor = addr;  //对端的地址信息
                        long time2 = System.currentTimeMillis();
                        double resolveAddrDelay = (time2 - time1) / 1000.00;
                        if (true) {

                            //request RDA of the other RDA
                            RDARequestMsg msg = new RDARequestMsg(me, true);  //me在begin方法里初始化
                            final long sendStamp = System.nanoTime();  //发送的时间戳
                            ...
                            comm.sendRequestMessage(msg, neighbor,
                                    new ObjCommRRCB<RDAResponseMsg>() {
                                        ...
                                        protected void cb(CBResult result, RDAResponseMsg arg1, AddressIF arg2, Long arg3) {
                                            // TODO Auto-generated method stub
                                            long receiveTS = System.nanoTime();  //回调回来接收的时间戳
                                            switch (result.state) {
                                                case OK: {
                                                    ...
                                                    RDATableEntry[] you = arg1._hashTable;  //回调回来的
                                                    //call compute
                                                    RDATable receiver = RDATable.getRDATable(you);  //将桶集合构造为bank
                                                    //ids
                                                    HashSet<Long> SenderItems = new HashSet<Long>();
                                                    HashSet<Long> ReceiverItems = new HashSet<Long>();
                                                    //decode
                                                    long decodeTS = System.nanoTime();
                                                    //该方法从receiver 与 sender中的subtract分离即丢的包和乱序包
                                                    boolean decoded = _hostA.decodeSet(receiver, SenderItems, ReceiverItems);//hostA 即sender   判断能否解码成功  并修改SenderItems(发送方丢的包) 和 ReceiverItems（接收方乱序包）
                                                    long d2 = System.nanoTime();
                                                    log.main("decoded: " + decoded + ", " + SenderItems.size() + ", " + ReceiverItems.size());

                                                    double decodeDelay = (d2 - decodeTS) / 1000000.0;  //解码时延
                                                    //no need to repair  因为丢的包 和 乱序包的hash表都为空
                                                    if (SenderItems.isEmpty() && ReceiverItems.isEmpty()) {
                                                        double[] doubles = computeStatistics(receiver);//直接计算平均时延  不用修复
                                                        System.out.println("avg "+doubles[0]+"----"+"statis: "+doubles[1]);
//                                                        System.out.println("resolveAddrDelay: " + resolveAddrDelay + "sendDelay: " + delay + ", decodeDelay: " + decodeDelay);
                                                        break;
                                                    } else {
                                                        //request the other's packets  有乱序的包
    ...
    public static void main(String[] args) {
        ...
        try {
            EL.get().main();//会执行 构造方法里 doRDARequest的回调
